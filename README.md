...
* After loading the data, export the state of your database using `pg_dump -U okcmy_applicant okc > dbexport.pgsql`. Include `dbexport.psql` in the `backend/scripts` folder.
...
* Within the `player-summary` component found in `frontend/src/my_app/player-summary/`, create an interface that describes the player summary data returned from the API.
...
2. Ensure PostgreSQL is running, and in a terminal run
    ```
    createuser okcmy_applicant --createdb;
    createdb okc;
    ```
3. connect to the okc database to grant permissions `psql okc`
    ```
    create schema my_app;
    alter user okcmy_applicant with password 'thunder';
    grant all on schema my_app to okcmy_applicant;
    ```
...
env virtualenv 3.10.1 okcThe steps below attempt to install Python version 3.10.1 within your pyenv environment. If you computer is unable to install this particular version, you can feel free to use a version that works for you, but note that you may also be required to update existing parts of the codebase to make it compatible with your installed version.
```
cd root/of/project
...
npm start
```
The frontend should run on http://localhost:4200/. Visit this address to see the my_app in your browser.
